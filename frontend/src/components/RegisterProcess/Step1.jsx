...
function Step1({ service_provider, type_service,  provider_description , updateFields }) {
  return (
    ...
    <input
          ...
          />
            ...
            <select  name="type_service"
             id="type_service"
              ...
              value={type_service}
              onChange={e => updateFields({ type_service: e.target.value })}
               className="appearance-none bg-gray-100 w-[200px] border  border-gray-300 text-gray-700 py-2 px-5 h-[40px] pr-10 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                <option value="Type of service" className='hidden' disabled={true}>provider category</option>
                ...
                <option value="transport">District </option>
          ...
          <textarea name=""
           ...
           id="" cols="30" rows="10"
            className='w-[80%] focus:border-blue-900 h-[100px] p-2 border-[1px] overflow-auto border-neutral-400 rounded-md'></textarea>
    </ModalWrapper>
  )
}
